...
(function ($, Drupal) {
   ...
   */
  Drupal.behaviors.s3corsAutoUpload = {

    attach: function (context, settings) {
      // Detach default Drupal file auto upload behavior from any s3fs cors file input elements.
      $(context).find('.s3cors-file input[type="file"]').removeOnce('auto-file-upload').off('.autoFileUpload');
      // Attach the custom s3fs cors auto upload processing behavior.
      $(context).find('.s3cors-file input[type="file"]').once('s3cors-auto-upload').on('change.s3corsAutoUpload', {settings: settings.s3fs_cors, baseUrl: settings.path.baseUrl}, Drupal.S3fsCors.triggerUploadButton);
    },
    detach: function (context, settings, trigger) {
      if (trigger === 'unload') {
        $(context).find('.s3cors-file input[type="file"]').removeOnce('s3cors-auto-upload').off('.s3corsAutoUpload');
      }
    }
  };
   ...
   *   Detaches triggers.
   */
  Drupal.behaviors.s3corsPreviewLinks = {
  ...
  Drupal.S3fsCors = Drupal.S3fsCors || {
     ...
     *   The event triggered. For example `change.s3corsAutoUpload`.
     */
                  ...
                  $.ajax({
                    ...
                    success: function (response, status, xmlHttpRequest) {
                      ...
                      for (var i = 0; i < responseLength; i++) {
                        ...
                        if (selector === null) {
                          var field_ajax_wrapper = $('div#edit-' + field_name_key.replace(/_/g, '-') + '-wrapper').find('div:first-child');
                          response[i].selector = '#' + field_ajax_wrapper.prop('id');
                        }
                      }
